...
import type { Lineatur } from '@/types';
...
export function Schreiblinie({ lineatur }: { lineatur: Lineatur }) {
  const m = LINEATUR_MASSE[lineatur];
...
export function Schreiblinien({ lineatur, anzahl = 1 }: { lineatur: Lineatur; anzahl?: number }) {
  return (
    <div className="flex flex-col gap-[3mm] py-[1.5mm]">
      {Array.from({ length: anzahl }, (_, i) => (
        <Schreiblinie key={i} lineatur={lineatur} />
      ))}
    </div>
  );
}
